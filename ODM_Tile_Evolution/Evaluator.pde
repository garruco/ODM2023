...
class Evaluator {
  ...
  float calculateFitness(Tile indiv) {
    PImage phenotype = indiv.getPhenotype(target_image.height, true);
    ...
    //float similarity = getSimilarity(target_pixels_brightness, phenotype_pixels_brightness);
    return similarity;
  }
  ...
  float getSimilarity(int[] sample1, int[] sample2)
  {
    float value = 0;
    float inc = 1/sample1.length;
    for (int i = 0; i < sample1.length; i++) {
      //if(sample1[i] > 0)println(i + "   " + sample1[i] + "   " + sample2[i]);
    }
    return value;
  }
