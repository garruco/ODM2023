...
class Tile {
  ...
  int genotype_length = tile_grid_side * tile_grid_side + 1;
  ...
  PImage getPhenotype(int resolution, boolean isBlack) {
  ...
  void export() {
    String output_filename = letter + "-" + tile_grid_side + "-" + year() + "-" + nf(month(), 2) + "-" + nf(day(), 2) + "-" +
                             nf(hour(), 2) + "-" + nf(minute(), 2) + "-" + nf(second(), 2);
    String output_path = sketchPath("outputs/" + output_filename);
    println("Exporting letter to: " + output_path);
    
    getPhenotype(resolution, false).save(output_path + ".png");
    
    PGraphics pdf = createGraphics(resolution, resolution, PDF, output_path + ".pdf");
    
    color currentColor = colors[floor(map(genes.get(0), 0, 1, 0, colors.length))];
    
    pdf.beginDraw();
    
    pdf.fill(currentColor);
    pdf.noStroke();
    render(pdf);
    pdf.dispose();
    pdf.endDraw();
