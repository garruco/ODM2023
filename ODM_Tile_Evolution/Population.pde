...
class Population {
  ...
  Population() {
    evaluator = new Evaluator(loadImage(letterPath), resolution);
  ...
  void evolve() {
    ...
    for (int i = elite_size; i < population_size; i++)
    {
      ...
      float current_fitness = evaluator.calculateFitness(next_generation[i]);
      next_generation[i].setFitness(current_fitness);
      individuals[i] = next_generation[i];
    }
